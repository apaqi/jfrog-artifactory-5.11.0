...
export class OnBoardingWizard {
    ...
    show(quickSetup = false) {
        ...
        whenReady.then(()=> {
            ...
            if (!quickSetup && !this.initStatus.hasLicenseAlready && !this.features.isAol() && !this.features.isNonCommercial()) steps.push({
...
class WizardController {
    ...
    initPackageTypesStep() {
        ...
        if (!this.gotPackageTypes) {
            this.OnboardingDao.reposStates().$promise.then((response)=> {
                ...
                this.packageTypes.packages = _.map(this.packageTypes.packages, (type)=> {
                    ...
                    let addTextBelowIcon = ['Bower', 'Chef', 'CocoaPods', 'Conan', 'Pypi', 'Puppet', 'Opkg', 'Composer', 'SBT', 'Gradle', 'Gems', 'NuGet', 'GitLfs','Generic','Go'];
                ...
                _.map(this.packageTypes.packages, (pack) => {
        ...
        this.packageTypes.checkNoResults = (typeFilter) => {
            if (typeFilter && typeFilter.length > 0 && _.filter(this.packageTypes.packages, (type)=>type.highlighted).length == 0) {
                return true;
            }
        };
