...
gulp.task('watch-essentials', (cb) => {
    fs.writeFile('./components/jfrog-ui-essentials/.announceBuildCompletion', '*', () => {
        gulp.watch(CONFIG.SOURCES.TEMPLATES, sequence('templates', 'reload'));
        gulp.watch(CONFIG.SOURCES.LESS, sequence('less', 'reloadCss'));
        gulp.watch('./components/jfrog-ui-essentials/.announceBuildCompletion', sequence('vendorScripts', 'vendorStyles', 'reload'));
        cb();
    })
});
...
gulp.task('watch-native-ui', (cb) => {
    fs.writeFile('./components/jfrog-native-ui/.announceBuildCompletion', '*', () => {
        gulp.watch(CONFIG.SOURCES.TEMPLATES, sequence('templates', 'reload'));
        gulp.watch(CONFIG.SOURCES.LESS, sequence('less', 'reloadCss'));
        gulp.watch('./components/jfrog-native-ui/.announceBuildCompletion', sequence('vendorScripts', 'vendorStyles', 'reload'));
        cb();
    })
});
