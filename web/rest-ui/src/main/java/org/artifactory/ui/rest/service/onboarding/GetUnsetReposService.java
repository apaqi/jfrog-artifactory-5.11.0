...
public class GetUnsetReposService implements RestService {
    ...
    private final static List<RepoType> conanCESupportedTypes = Lists.newArrayList(
            RepoType.Generic,
            RepoType.Conan
    );
    ...
    private Map<RepoType, OnboardingRepoState> getSupportedRepoTypesMap() {
        ...
        if (isOss) {
            if (addonsManager.getArtifactoryRunningMode().equals(ArtifactoryRunningMode.CONAN)) {
