...
public class SecurityServiceImpl implements InternalSecurityService {
    ...
    private EncodedKeyPair convertOldKeyFormatIfNeeded(UserInfo user) {
        ...
        EncodedKeyPair toSaveEncodedKeyPair = encodedKeyPair.toSaveEncodedKeyPair(null);
    ...
    private boolean hasPermission(RepoPath repoPath, ArtifactoryPermission permission) {
        ...
        if (repoPath.getRepoKey().equals(DistributionConstants.IN_TRANSIT_REPO_KEY) && ArtifactoryPermission.READ.equals(permission)) {
        ...
        if (isAnonymous() && !isAnonAccessEnabled()) {
            log.debug("Annonymous but AnonAccess disabled for "+repoPath);
            return false;
        }
        ...
        boolean granted = isGranted(repoPath, permission, sids);
        boolean grantedByAccessToken = isGrantedByAccessToken(repoPath, authentication);
        boolean isGranted = granted || (ArtifactoryPermission.READ.equals(permission) && grantedByAccessToken);
        log.debug("Access granted: "+granted+". granted by access token: "+grantedByAccessToken+" for "+repoPath);
        return isGranted;
    }
