    ...
    public DirContextOperations authenticate(Authentication authentication) {
        ...
        if (user == null) {
            if (authenticationException) {
                throw new AuthenticationServiceException("User " + username + " failed to authenticate");
            }
            // if all of the searches returned UsernameNotFoundException
            if (userSearches != null && notFoundCount == userSearches.size()) {
                log.debug("User '" + username + "' can't be found in LDAP search");
                throw new UsernameNotFoundException("user " + username + " can't be found in LDAP search");
            }
