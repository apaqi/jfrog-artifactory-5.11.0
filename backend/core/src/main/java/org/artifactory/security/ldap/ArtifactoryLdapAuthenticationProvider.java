...
public class ArtifactoryLdapAuthenticationProvider implements RealmAwareAuthenticationProvider, MessageSourceAware {
        ...
        try {
            ...
            if (user == null) {
                if (authenticationException != null) {
                    log.debug("Failed to authenticate user '" + userName + "' using ldap.",
                            authenticationException.getMessage());
                    if (shouldRemoveUserLdapRelatedGroups(authenticationException) &&
                            userSetForGroupDeletion(userName)) {
    ...
    /**
     * User will have his LDAP groups removed if he is locked or disabled, not exist in LDAP or no LDAP has been configured
     */
    private boolean userSetForGroupDeletion(String username) {
        List<LdapSetting> settings = centralConfig.getMutableDescriptor().getSecurity().getLdapSettings();
        if (settings == null || settings.isEmpty()) {
            log.debug("No LDAP settings defined");
            return true;
        ...
        for (LdapSetting setting : settings) {
            if (setting.isEnabled()) {
                ...
                if (ldapUser != null) {
                    ...
                    String userAccountControl = (userAccountControlAttr != null) ? userAccountControlAttr.toString() : null;
        ...
        // user has not been found in ldap -> ldap groups should be deleted
        return true;
    }
}