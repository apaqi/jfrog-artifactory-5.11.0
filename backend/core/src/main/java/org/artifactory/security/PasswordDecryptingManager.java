...
public class PasswordDecryptingManager implements AuthenticationManager {
    ...
    private EncryptionWrapper getEncryptionWrapper(String username) {
        ...
        EncodedKeyPair newEncodedKeyPair = encodedKeyPair.toSaveEncodedKeyPair(null);
        ...
        DecodedKeyPair decodedKeyPair;
        try {
            decodedKeyPair = encodedKeyPair.decode(null, decryptionStatus);
        } catch (Exception e) {
            if (log.isTraceEnabled()) {
                log.trace("Trying to decode with null", e);
            }
            decodedKeyPair = encodedKeyPair.decode(artifactoryKeyWrapper, decryptionStatus);
        }
        reEncryptKeysIfNeeded(userInfo, artifactoryKeyWrapper, decryptionStatus, decodedKeyPair);
    ...
    private void reEncryptKeysIfNeeded(UserInfo userInfo, EncryptionWrapper artifactoryKeyWrapper,
        ...
        if (decryptionStatus.hadFallback()) {
            ...
            encodedKeyPair = new EncodedKeyPair(decodedKeyPair, artifactoryKeyWrapper);
