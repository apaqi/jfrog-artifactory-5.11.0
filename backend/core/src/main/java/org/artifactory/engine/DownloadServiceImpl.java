...
public class DownloadServiceImpl implements InternalDownloadService {
    ...
    public void process(ArtifactoryRequest request, ArtifactoryResponse response) throws IOException {
        ...
        try {
            ...
            if (repository == null) {
                ...
                // if edge licensed and not virtual(real) and either remote(not local) or cache, throw unsupported exception
                if ((addonsManager.isEdgeLicensed() || addonsManager.isEdgeMixedInCluster()) &&
                        (repository.isReal() && (!repository.isLocal() || repository.isCache()))) {
                    throw new UnsupportedByLicenseException("Remote repository is not support in Edge ");
                }
    ...
    private void respondForChecksumRequest(Request request, ArtifactoryResponse response,
        ...
        if (request.isHeadOnly()) {
            ...
            requestResponseHelper.sendBodyResponse(response, requestRepoPath, checksum, request, resource.isExpirable());
        }
    }
