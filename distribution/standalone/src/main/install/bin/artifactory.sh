...
# Start replicator if exists
startReplicator() {
    if [ "${REPLICATOR_ENABLED}" == true ] && [ -f ${replicatorScript} ]; then
        chmod +x ${replicatorScript}
        ${replicatorScript} start
    fi
}

# Stop replicator if exists
stopReplicator() {
    if [ "${REPLICATOR_ENABLED}" == true ] && [ -f ${replicatorScript} ]; then
        chmod +x ${replicatorScript}
...
export REPLICATOR_HOME="$(cd "$(dirname "${artBinDir}")/replicator" && pwd)"
artDefaultFile="$artBinDir/artifactory.default"
replicatorScript=${REPLICATOR_HOME}/bin/replicator.sh
