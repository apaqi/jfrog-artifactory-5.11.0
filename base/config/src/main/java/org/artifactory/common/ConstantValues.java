...
public enum ConstantValues {
    ...
    xrayClientBlockUnScannedCacheExpirationIntervalSecs("xray.client.block.unscanned.cache.expiration.intervalSecs", 120),
    ...
    xrayClientNormalSocketTimeoutMillis("xray.client.normal.socket.timeout.millis", 5000),
    ...
    minReplicatorUseFileSizeInBytes("min.replicator.use.filesize.in.bytes", 100_000),
     ...
     */
    nodeEventsEnabled("node.events.enabled", FALSE),
    /**
     * whether to enable recording of tree node event metrics
     */
    nodeEventsMetricsEnabled("node.events.metrics.enabled", FALSE),
    /**
     * max time in millis to keep node events records
     */
    nodeEventsMaxTime("node.events.max.time", TimeUnit.DAYS.toMillis(365)),
    releaseBundleCleanupMaxAgeHours("releasebundle.cleanup.maxAgeHours", 0),
    ...
    csrfFilterEnable("csrf.filter.enabled", FALSE),
